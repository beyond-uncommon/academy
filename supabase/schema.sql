...
-- ═════════════════════════════════════════════════════════════════
-- Post-schema migrations (additions that were created in separate
-- migration files but are included here for a complete bootstrap)
-- ═════════════════════════════════════════════════════════════════

-- ─── Feedback column on project_submissions ──────────────────
alter table public.project_submissions add column if not exists feedback text;

-- ─── Graduation flag on profiles ─────────────────────────────
...
create policy if not exists "certificates_read_all" on public.certificates
  for select using (true);
...
create policy if not exists "Anyone can read community posts"
  on public.community_posts for select
  to authenticated
  using (true);
create policy if not exists "Users can create community posts"
  on public.community_posts for insert
  to authenticated
  with check (auth.uid() = user_id);
create policy if not exists "Users can update own community posts"
  on public.community_posts for update
  to authenticated
  using (auth.uid() = user_id)
  with check (auth.uid() = user_id);
create policy if not exists "Users can delete own community posts"
...
create policy if not exists "Anyone can read post likes"
  on public.post_likes for select
  to authenticated
  using (true);
create policy if not exists "Users can manage own post likes"
  on public.post_likes for insert
  to authenticated
  with check (auth.uid() = user_id);
create policy if not exists "Users can remove own post likes"
  on public.post_likes for delete
  to authenticated
  using (auth.uid() = user_id);

-- ─── Extend lesson_comments with post_id ─────────────────────
...
-- ─── Leaderboard: allow anyone to read profiles ──────────────
create policy if not exists "profiles_read_authenticated" on public.profiles
  for select
  to authenticated
  using (true);
